...
// Type definitions for our dependencies
type JyotishCalculations = {
  grahas: {
    getGrahasPosition: (details: any) => any
  }
  rashis: {
    getRashi: (longitude: number) => string
  }
}
...
let jyotish: { default: JyotishCalculations } | null = null
try {
  jyotish = await import('jyotish-calculations') as any
} catch (e) {
  console.error('❌ jyotish-calculations not installed:', (e as any).message)
}
...
try {
  ...
  console.warn('⚠️ swisseph-v2 not installed:', (e as any).message)
}
...
app.use('*', async (c, next) => {
  ...
  if (c.req.method === 'OPTIONS') {
    return c.text('', 200, CORS_HEADERS)
  }
...
app.post('/api/rashi', async (c) => {
  try {
    ...
    if (isNaN(date.getTime())) return c.json({ error: 'Invalid date format' }, 400, CORS_HEADERS)
    ...
    return c.json({ error: 'Failed to calculate rashi', message: (error as any).message }, 500, CORS_HEADERS)
  }
})
